...
vec3 getNormal(PBRParams params)
{
	...
	{
		return params.hasNormalTexture ? texture(tex_normal, var_texcoord0).rgb : vec3(0.0);
	}
#endif
	...
	{
		lowp vec3 sample_normal = texture(tex_normal, var_texcoord0).rgb;
...
MaterialInfo getMaterialInfo(PBRParams params)
{
	...
	{
		lowp vec4 sample_albedo = texture(tex_albedo, var_texcoord0);
	...
	{
		lowp vec4 sample_roughness       = texture(tex_metallic_roughness, var_texcoord0);
