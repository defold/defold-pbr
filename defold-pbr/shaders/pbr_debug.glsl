...
#define PBR_DEBUG
	...
	#define GET_DEBUG_MODE() int(u_pbr_scene_params.x)
		...
		vec4 applyDebugMode(vec4 color_in, MaterialInfo materialInfo, LightingInfo lightInfo, PBRData pbrData)
		{
			...
			else if (debug_mode == DEBUG_MODE_TC_0)           return vec4(mod(var_texcoord0, 1.0), 0.0, 1.0);
			else if (debug_mode == DEBUG_MODE_TC_1)           return vec4(mod(var_texcoord1, 1.0), 0.0, 1.0);
